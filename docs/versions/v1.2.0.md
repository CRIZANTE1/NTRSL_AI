...
```bash
npx cap add android    # primeira vez — gera pasta android/
npm run cap:sync       # build web + sync
npm run cap:open       # abre no Android Studio
```

APK/AAB: compile no **Android Studio** (não via `gradlew` no terminal). Ver [ANDROID.md](../ANDROID.md).
...
| APK de release | Gerado e assinado no Android Studio — ver [ANDROID.md](../ANDROID.md) |
